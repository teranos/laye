...
resource "aws_route53_record" "bevy_starter" {
  zone_id = data.aws_route53_zone.root.zone_id
  name    = local.bevy_starter_fqdn
  type    = "A"

  alias {
    name                   = aws_cloudfront_distribution.bevy_starter.domain_name
    zone_id                = aws_cloudfront_distribution.bevy_starter.hosted_zone_id
    evaluate_target_health = false
  }
}

resource "aws_route53_record" "bevy_starter_cert_validation" {
  for_each = {
    for dvo in aws_acm_certificate.bevy_starter.domain_validation_options : dvo.domain_name => {
      name   = dvo.resource_record_name
      record = dvo.resource_record_value
      type   = dvo.resource_record_type
    }
